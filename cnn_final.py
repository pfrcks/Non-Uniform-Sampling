...
val_vals = []
duration_vals = []
f = open('results/cnn_'+str(args.dataset)+'_'+str(args.sample_type)+'.train.log', 'w')
ff = open('results/cnn_'+str(args.dataset)+'_'+str(args.sample_type)+'.test.log', 'w')
fff = open('results/cnn_'+str(args.dataset)+'_'+str(args.sample_type)+'.val.log', 'w')
ffff = open('results/cnn_'+str(args.dataset)+'_'+str(args.sample_type)+'.dur.log', 'w')

if args.sample_type == 'grad':
    for epoch in np.arange(3):
        start = time.time()
        uniform_sampling(x_train, y_train)
        duration_vals.append(time.time() - start)
        train_vals.append(test(x_train, y_train))
        test_vals.append(test(x_test, y_test))
        val_vals.append(test(x_val, y_val))
    start_grad = time.time()
    weights = gradients(x_train, y_train)
    total_grad = time.time() - start_grad
    weights = weights.reshape(weights.shape[0], 1)
    weights = np.column_stack([weights, range(weights.shape[0])])
    prob = args.batch_size * (weights[:, 0] / np.sum(weights[:, 0]))
    for epoch in np.arange(args.epochs-3):
        ...
        test_vals.append(test(x_test, y_test))
        val_vals.append(test(x_val, y_val))
    duration_vals.append(total_grad)
elif args.sample_type == 'obj':
    start = time.time()
    loss, losses = train(x_train, y_train, ret_all_losses=True)
    duration_vals.append(time.time() - start)
    ...
    for epoch in np.arange(args.epochs):
        ...
        val_vals.append(test(x_val, y_val))
f.write(str(train_vals))
ff.write(str(test_vals))
fff.write(str(val_vals))
ffff.write(str(duration_vals))
