...
def train2(x_train, y_train):
    model.train()
    x_train = torch.from_numpy(x_train.copy()).type(torch.FloatTensor)
    y_train = torch.from_numpy(y_train.copy()).type(torch.LongTensor)
    if args.cuda:
...
for epoch in range(10):
    ...
    for i in range(60000/args.batch_size):
        ...
        x_batch_var = np.var(x_batch.reshape(64, 784), axis=1)
        ind = np.argsort(x_batch_var)
        x_batch = x_batch[ind]
        y_batch = y_batch[ind]
        losses = train2(x_batch[:32], y_batch[:32])
        # losses = train2(x_train[i*64:(i+1)*64][:32], y_train[i*64:(i+1)*64][:32])
    test2(x_test, y_test)
