...
def train(x, y, ret_all_losses=False):
    ...
    all_losses = None
    if ret_all_losses:
        all_losses = F.cross_entropy(output, target, reduce=False)
        if args.cuda:
...
def gradient_sampling(x, y, prob):
    ...
    for i in range(n/size):
        sel = np.random.binomial(1, prob)
        x_batch = x_train[np.where(sel == 1)]
        y_batch = y_train[np.where(sel == 1)]
...
def objective_sampling(x, y, losses):
    ...
    for i in range(n/size):
        ...
        idx = losses[:, 1][:size]
        idx = idx.astype(int)
        loss, loss_batch = train(x_train[idx], y_train[idx], ret_all_losses=True)
